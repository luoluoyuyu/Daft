...
def gen_parquet(csv_files_location: str) -> str:
    ...
    for tab_name in SCHEMA.keys():
        ...
        if not os.path.exists(table_parquet_path):
            ...
            os.makedirs(table_parquet_path, exist_ok=True)
            ...
            for file in files:
                ...
                # dbgen emits a trailing delimiter that produces an empty last column
                if arrow_table.num_columns > len(SCHEMA[tab_name]):
                    arrow_table = arrow_table.drop_columns([arrow_table.column_names[-1]])
                arrow_table = arrow_table.rename_columns(SCHEMA[tab_name])
                papq.write_table(arrow_table, os.path.join(table_parquet_path, os.path.basename(file) + ".parquet"))
        else:
            logger.info(
